run_mse = function(n_sims, nyears, nages, init_nya,
                   ...
                   sca_model_path = NULL, spm_model_path = NULL, lbspr_model_path = NULL, 
                   maturity, thresholds, vb_params, plot = FALSE,
                   estimation = FALSE, est_method = "SCA") {
  ...
  if (estimation == TRUE) {
    ...
    if (est_method == "SCA") {
      if (is.null(sca_model_path)) {
        stop("Estimation is TRUE (SCA) but 'sca_model_path' is not provided.")
      }
    ...
    if (est_method == "SPM") {
      if (is.null(spm_model_path)) {
        stop("Estimation is TRUE (SPM) but 'spm_model_path' is not provided.")
      }
    ...
    if (est_method == "LBSPR") {
      if (is.null(lbspr_model_path)) {
        stop("Estimation is TRUE (LBSPR) but 'lbspr_model_path' is not provided.")
      }
  ...
  if (plot == TRUE && estimation == FALSE) {
    par(mfrow = c(2,3), mar = c(3,3,2,1), oma = c(1,1,1,1), mgp = c(2,0.7,0), tcl = -0.3)
    ...
    for(i in 1:10) { lines(abs_biomass_mat[,i], col = rgb(0,0,0,0.2)) }
    ...
    for(i in 1:10) { lines(rel_biomass_mat[,i], col = rgb(0,0,0,0.2)) }
    ...
    for(i in 1:5) { lines(rec_mat[,i], col = rgb(0,0,0,0.2)) }
    abline(v = burn_in_length, lty = 2, col = "blue")
    box(lwd = 2)
    
    plot(catch_mean, type = "p", pch = 20, cex = 0.8, xlab = "Year", ylab = "Catch (t)", ylim = c(0, max( (catch_mean + catch_sd)*1.2)))
    lines(catch_mean + catch_sd*1.92, lty = 2)
    lines(catch_mean - catch_sd*1.96, lty = 2)
    abline(v = burn_in_length, lty = 2, col = "blue")
    box(lwd = 2)
    
    plot(mean_lenghts_mu, type = "l", pch = 20, cex = 1, xlab = "Age", ylab = "Mean length (cm)", ylim = c(0, max( (mean_lenghts_mu + mean_lengths_sd)*1.2)))
